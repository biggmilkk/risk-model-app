...
def calculate_risk_summary(inputs: list[RiskInput], critical_alert: bool=False):
    ...
    counts = Counter([r.category for r in high])
    counts.update({cat: cnt for cat, cnt in Counter([r.category for r in mid]).items() if cnt >= 5})
    qualifying = [cat for cat, cnt in counts.items() if cnt >= 1]
    cluster_bonus = 2 if len(qualifying) >= 3 else 1 if qualifying else 0
...
def advice_matrix(score: int) -> dict[str, str]:
    ...
    }.get(score, {"Low": "NA", "Moderate": "NA", "High": "NA"})
